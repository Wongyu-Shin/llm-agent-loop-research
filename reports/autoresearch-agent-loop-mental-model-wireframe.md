...
- **Upp 히트맵**: 지도 배경을 수렴 정확도에 따라 칠한다 — 어두울수록 0%, 네온 그린에 가까울수록 100%. Upp는 `(1,0)` 코너 기준 각도만의 함수이므로 Upp 등간격 쐐기 팬으로 정확히 채색되며, 고Upp 밴드가 코너 옆에서 얇게 압축되는 모습 자체가 "좁은 쐐기" 메시지다.
- **부챗살 등고선**: 목적지 50/75/90/99% 직선이 `(1,0)` 코너에서 방사(금색 점선, 정적). 축은 [0,1] 전체 — 높은 기대 쐐기의 얇음 자체가 메시지이므로 확대하지 않는다.
- **수렴 파동(상시 애니메이션)**: 경계선(시작 수위)에서 양방향으로 파동이 퍼진다 — 점등 쪽은 높은 목적지 방향, 소등 쪽은 낮은 목적지 방향. 각 위치의 정확도가 Acc₀에서 자기 목적지로 이동함을 지속적으로 보여 준다(reduced-motion 시 정지).
- **호버 인디케이터**: 지도 위 임의 지점에 마우스를 올리면 그 위치의 `CL`·`CS` 정확값과 수렴 정확도 `Upp` 라벨, 해당 등고선이 표시되고, 목적지 게이지가 그 값을 따라간다. 같은 정보는 등고선·표로도 제공된다.
...
- **경계선**(`Upp=Acc_0`): 히트맵의 네온 그린과 겹치지 않도록 시안(accent) 네온 — 게이지의 `Acc_0` 마커와 같은 색으로 "현재 수위의 등고선"이라는 의미를 색으로도 잇는다.
...
- **키보드 탐색**: 실측 8점이 focus 가능한 44px 핫스팟이며, 선택 지점의 `Upp`가 목적지 게이지에 반영된다.
...
#### 데이터 선반 — 모든 데이터, 한 화면 금지, 개형 기준 그루핑

- 원문 전 곡선(Figure 1·3·4·5·6·7·8·9·10, 약 40개)을 **개형(행동) 기준** 그룹의 스파크라인 칩으로 수록: 상승·포화(19), 하락(자연 3 + Are you sure 1), CL=1 돌파(8), 따름정리(팬차트 3·α 비교 6). 출처 그림 번호는 그룹 라벨에 병기한다.
- **전체 개형 스트립**: 무대 아래에 팬차트(인위 조작)를 제외한 실측 37곡선을 겹쳐 그린다. 좌표는 토글 — `실측 %`(원좌표)와 `수렴 진행도 (Accₜ − Acc₀)/(Upp − Acc₀)`(모든 곡선이 0→1로 접근하는 한 가족, 잔여 편차 = α^t의 시각 증명). 진행도 뷰에서는 |Upp − Acc₀| 3%p 미만이라 정규화 노이즈만 증폭되는 케이스를 제외하고 제외 수를 fallback에 명시한다. 선택 케이스는 강조, 같은 개형 그룹은 중간 강조, 나머지는 실루엣.
...
1. 6초 동안 무대 구조를 소개한다 — 목적지 선, 보정/예측 구간, 실측·이론의 구분.
2. 8초 동안 상승·포화 그룹의 대표 GSM8k 케이스를 리플레이한다.
3. 7초 동안 하락 그룹의 대표 Are you sure 케이스를 리플레이한다.
4. 6초 동안 CL=1 돌파 그룹의 대표 oracle 케이스를 리플레이한다.
5. 7초 동안 전체 개형 스트립을 수렴 진행도 좌표로 전환해 모든 곡선이 한 가족임을 보여 준다.
6. 3초 동안 `직접 도출 / 실험이 지지 / 아직 미검증` 세 열 overlay를 강조한다.
7. 3초 동안 Part I 종료 문장과 PartBoundary를 표시한다.
