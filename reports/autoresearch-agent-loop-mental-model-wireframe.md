...
| I | 3 | 한 라운드는 언제 순이득인가 | 19 | 2:32 | 0:28 | 3:00 | 7:40 |
| I | 4 | 반복하면 어디로 가며 논문은 어디까지 검증했는가 | 25 | 3:20 | 0:40 | 4:00 | 11:40 |
...
## 5. Scene 3 — 복구 이득이 훼손 손실보다 커야 한다
...
>
  <Title>
    <h2>한 라운드의 순변화를 이득과 손실로 나눈다</h2>
    <p>초기 정확도가 높을수록 작은 훼손률의 비용이 커진다.</p>
  </Title>

  <RepairDamageLab
    primary
    presets={["balanced", "99-percent-accuracy", "harmful-revision"]}
    autoplay="99-percent-accuracy"
  />

  <PersistentConclusion badge="PAPER">
    수정은 CS_t가 높다는 이유만으로 유리하지 않으며,
    현재 정확도에 가중된 훼손까지 이겨야 한다.
  </PersistentConclusion>
</Scene>
```
...
`Acc_t=0.99`, `CS_t=0.50`이라고 가정한다.

| 항목 | 계산 | 전체에 대한 비율 |
| --- | --- | ---: |
| 남은 오답 | `1 - 0.99` | 1.00% |
| 복구 이득 | `0.01 × 0.50` | 0.50% |
| 손익분기 `CL_t` | `1 - 0.005 / 0.99` | 약 99.49% |

기존 정답의 약 0.51%보다 많이 훼손하면 남은 오답의 절반을 복구해도 전체 정확도는 내려간다. 이 수치는 논문의 실험에서 직접 측정한 결과가 아니라 전이식에 값을 넣어 얻은 수학적 예시다.

### 5.4 Primary SVG — `RepairDamageLab`
...
- `Acc_t`, `CL_t`, `CS_t` slider.
- `손익분기 CL_t 계산` 버튼.
- preset: 낮은 초기 정확도, 99% 정확도, 복구보다 훼손이 큰 사례.
- 상태: `BENEFICIAL`, `BREAK_EVEN`, `HARMFUL`.
- 항상 raw equation과 대입한 값을 함께 표시.

#### 자동 재생과 28초 시각 예산

1. 6초 동안 기본 순변화 막대를 읽는다.
2. 14초 동안 99% preset에서 `CL_t`를 100%에서 99.40%까지 낮춰 sign이 바뀌는 순간을 보여 준다.
3. 4초 동안 break-even 값을 고정한다.
4. 4초 동안 반복 궤적 화면으로 전환한다.

#### 항상 보이는 결론

> 좋은 수정 정책은 오답을 많이 고치는 정책이 아니라 복구보다 더 큰 훼손을 만들지 않는 정책이다.

#### 접근성 DOM 대체

- 입력값, 복구 이득, 훼손 손실, 순변화, verdict를 실제 표로 제공한다.
- 양수와 음수는 색뿐 아니라 `+`, `−`, `BENEFICIAL`, `HARMFUL` text로 구분한다.
- break-even 계산은 `<output>`으로 연결하고 소수점 반올림 전 값을 보조 설명으로 제공한다.

### 5.5 발표자 노트

<PresenterNotes scene="03" sentenceBudget={19} speechSeconds={152} visualSeconds={28}>

- S03.01 한 라운드의 순변화는 다음 정확도에서 현재 정확도를 빼면 드러납니다.
- S03.02 정리하면 순변화는 1 빼기 Acc_t에 CS_t를 곱한 값에서 Acc_t에 1 빼기 CL_t를 곱한 값을 뺀 결과입니다.
- S03.03 첫 항은 기존 오답 가운데 이번 수정으로 되찾은 복구 이득입니다.
- S03.04 둘째 항은 기존 정답 가운데 이번 수정으로 잃은 훼손 손실입니다.
- S03.05 평균 정확도가 오르려면 복구 이득이 훼손 손실보다 커야 합니다.
- S03.06 화면에서는 복구 이득과 훼손 손실의 대소가 바뀌는 순간을 손익분기로 표시합니다.
- S03.07 CL_t에 대한 손익분기값은 현재 정확도와 CS_t를 넣어 바로 계산할 수 있습니다.
- S03.08 초기 정확도가 높아질수록 복구할 오답의 몫은 작아지고 보호해야 할 정답의 몫은 커집니다.
- S03.09 그래서 강한 초기 응답일수록 작은 훼손률도 복구 이득을 쉽게 상쇄할 수 있습니다.
- S03.10 이제 현재 정확도가 99퍼센트이고 CS_t가 0.5인 예를 보겠습니다.
- S03.11 전체 응답에서 남아 있는 오답은 1퍼센트입니다.
- S03.12 그 오답의 절반을 복구해도 전체 정확도의 이득은 0.5퍼센트포인트입니다.
- S03.13 반면 기존 정답의 훼손 손실은 0.99 곱하기 1 빼기 CL_t입니다.
- S03.14 이득을 내려면 0.005가 0.99 곱하기 1 빼기 CL_t보다 커야 합니다.
- S03.15 따라서 손익분기 정답 보존율은 약 99.49퍼센트입니다.
- S03.16 이는 기존 정답의 약 0.51퍼센트만 훼손해도 복구 이득이 사라진다는 뜻입니다.
- S03.17 정확히 손익분기점에서는 수정 전후의 평균 정확도가 같습니다.
- S03.18 예를 들어 CL_t가 99퍼센트라면 다음 정확도는 98.51퍼센트로 0.49퍼센트포인트 내려갑니다.
- S03.19 이 수치는 특정 모델의 측정 결과가 아니라 전이식의 위험 구조를 보여주는 계산 예시입니다.

</PresenterNotes>

**컷 후보:** `S03.07`, `S03.15`를 제외하고 99% preset 정지 시간을 각각 8초 늘릴 수 있다.

## 6. Scene 4 — 반복 모형의 예측력과 한계를 함께 읽는다
...
>
  <Title>
    <h2>고정점은 예측 도구이지 보편적 성능 보장이 아니다</h2>
    <p>정지 가정, 실험 범위, 보고되지 않은 불확실성을 함께 본다.</p>
  </Title>

  <ScalingEvidenceExplorer
    primary
    views={["trajectory", "experiment", "claim-boundary"]}
    autoplay="three-views-once"
  />
...
논문은 `CL_t`와 `CS_t`가 라운드와 무관한 상수 `CL`, `CS`라고 가정해 다음을 얻는다. 정확도, 수렴 상한, 수렴 계수는 논문의 `Acc_t`, `Upp`, `α` 표기를 그대로 쓴다.
...
### 6.5 `ScalingEvidenceExplorer`

#### 세 view

1. `trajectory`: 같은 `Upp`를 기준으로 `Acc_0<Upp`, `Acc_0=Upp`, `Acc_0>Upp`의 세 곡선을 겹쳐 보여 준다.
2. `experiment`: 첫 전이의 추정값에서 2~5라운드 이론 곡선이 만들어지는 절차를 보여 준다.
3. `claim-boundary`: `직접 도출`, `실험이 지지`, `아직 미검증` 카드를 세 열로 분리한다.

#### 자동 재생과 40초 시각 예산

1. 16초 동안 세 초기 정확도 궤적을 함께 재생한다.
2. 10초 동안 첫 전이에서 이후 라운드를 예측하는 실험 절차를 재생한다.
3. 10초 동안 claim-boundary의 세 열을 순서대로 강조한다.
4. 4초 동안 Part I 종료 문장과 PartBoundary를 표시한다.

#### 항상 보이는 결론

> 고정점은 정지 모형의 결과이며, 적용 범위는 논문의 데이터와 절차만큼만 넓다.

#### 접근성 DOM 대체

- trajectory는 라운드별 `Acc_t` 값의 데이터 표로 제공한다.
- 각 곡선의 시작값, `Upp`, `α`, 방향을 text summary로 제공한다.
...
- S04.01 여러 라운드의 닫힌식을 얻으려면 CL과 CS가 반복 동안 일정하다고 추가로 가정해야 합니다.
- S04.02 이 가정 아래 다음 정확도는 CL 빼기 CS에 현재 정확도를 곱한 뒤 CS를 더한 값입니다.
- S04.03 논문이 Upp로 쓰는 정확도 수렴 상한은 CS를 1 빼기 CL 더하기 CS로 나눈 값입니다.
- S04.04 다만 CL이 1이고 CS가 0이면 분모가 0이므로 단일 고정점 공식을 적용하지 않습니다.
- S04.05 Upp에서는 오답 복구 이득과 정답 훼손 손실이 정확히 균형을 이룹니다.
- S04.06 논문이 주로 다루는 α가 0과 1 사이인 구간에서는 현재 정확도가 Upp보다 낮으면 평균 궤적이 Upp를 향해 올라갑니다.
- S04.07 같은 구간에서 현재 정확도가 Upp보다 높으면 평균 궤적은 오히려 Upp를 향해 내려갑니다.
- S04.08 논문은 Upp를 upper bound라고 부르지만 수학적으로는 점근적 고정점이라고 읽는 편이 정확합니다.
- S04.09 따라서 Upp는 모델의 절대 능력 상한이 아니라 특정 데이터와 프롬프트와 수정 절차가 만드는 목적지입니다.
- S04.10 수렴 계수 α는 CL 빼기 CS로 정의됩니다.
- S04.11 논문의 닫힌식은 Acc_t가 Upp에서 α의 t제곱과 Upp 빼기 Acc_0을 곱한 값을 뺀 결과라고 씁니다.
- S04.12 고정점까지의 편차는 라운드마다 α배가 되므로 α의 절댓값이 수렴 속도를 정합니다.
- S04.13 작은 α의 절댓값은 더 빠른 접근을 뜻할 뿐 더 높은 고정점을 보장하지 않습니다.
- S04.14 α가 0과 1 사이면 단조롭게 접근하지만 음수면 고정점 양쪽을 오갈 수 있습니다.
- S04.15 저자들은 초기 정확도 Acc_0과 첫 수정에서 얻은 CL과 CS로 이후 정확도 곡선을 계산했습니다.
- S04.16 그리고 같은 reask 절차를 실제로 다섯 라운드까지 반복해 예측한 2회차부터 5회차의 궤적과 비교했습니다.
- S04.17 각 질문에서는 응답 다섯 개를 독립적으로 샘플링하고 평균 정확도를 사용했습니다.
- S04.18 실험에는 오픈소스 모델 다섯 개와 Qwen-Max와 GPT-3.5 Turbo와 GPT-4 Turbo를 합친 여덟 모델이 포함되었습니다.
- S04.19 데이터셋은 GSM8K와 HumanEval과 MMLU 등을 포함해 생성 과제와 분류 과제를 섞은 여덟 종류였습니다.
- S04.20 다만 여덟 모델과 여덟 데이터셋이 만드는 64개 조합을 모두 실행한 것은 아닙니다.
- S04.21 Llama3-8B에서는 다섯 라운드 동안 CL과 CS가 대체로 안정적인 사례도 확인했습니다.
- S04.22 여러 설정의 상승과 포화 모양은 이론 곡선과 정성적으로 비슷했고 Are you sure 프롬프트의 하락 사례도 설명되었습니다.
- S04.23 첫 수정 뒤의 Acc_1은 같은 전이 데이터에 맞춰지므로 미래 예측력은 주로 2회차 이후에서 판단해야 합니다.
- S04.24 논문은 곡선을 시각적으로 비교했지만 평균제곱근오차(RMSE), 결정계수(R²), 신뢰구간 같은 정량 적합도 지표는 보고하지 않았습니다.
- S04.25 결과는 다섯 라운드의 유용한 일차 근사를 지지할 뿐 보편 법칙이나 이후 반복을 입증하지 않습니다.

</PresenterNotes>

**컷 후보:** `S04.11`, `S04.17`, `S04.21`을 제외하고 trajectory와 claim-boundary 관찰 시간을 각각 8초 늘릴 수 있다.
...
│                                                           │
│ NOW  S04.18 실험에는 오픈소스 모델 다섯 개와…             │
│ NEXT S04.19 데이터셋은 GSM8K와 HumanEval과…                │
│                                                           │
...
- Scene 4에서 수학, 실험 관찰, 비판적 한계를 모두 설명한 뒤 Scene 5가 full-width 경계를 표시한다.
...
- `TwoStateTransitionLab`, `RepairDamageLab`, `ScalingEvidenceExplorer`, `AutoresearchLoopMap`, `LoopControlWorkbench`가 서로 다른 학습 목표를 가진다.
...
4. `TwoStateTransitionLab`, `RepairDamageLab`, `ScalingEvidenceExplorer`와 DOM 대체를 구현한다.
